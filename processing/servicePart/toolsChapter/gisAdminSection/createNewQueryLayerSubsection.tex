  %
  %
  %  Or Can compile alone with the commented texts toggled
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
%    To Do:
%
%
%
%-------------------------------------------------------------------
% OPTIONAL PREAMBLE FOR LOCAL COMPILE  %
  %
% \def\titlename{Query Layers, Creating Queries in ArcGIS}
% \def\authorName{Allegan County GIS Services}
% \def\pdfTitle{Query Layers, Creating Queries in ArcGIS}
% \def\pdfSubject{GIS Tools} %
% \def\pdfKeywords{mobile,gis}
%
% \input{../../../preamble/subSectionPreamble.tex}
  %
%-------------------------------------------------------------------
    %
    %
\begin{document}% document begins
 %
 %--------------------------------------------------
  %
\subsection[Create Query in ArcGIS to SQL Database]{Create Query {\tiny to SQL Database} in ArcGIS}
\medskip
\subsubsection[Add Query Layer ]{\Large Add Query Layer}
\paragraph*{In ArcMap: \texorpdfstring{\\}{}}
Open the New Query Layer Dialog\\
Go to $\Rightarrow$ File $\Rightarrow$ Add Data $\Rightarrow$ Add Query Layer
In the connection dropdown select your connection
\paragraph*{}NOTE
\begin{figure}[H]
\centering
    \includegraphics[width=.75\textwidth]{NewQueryLayerDialog.png}
\caption{New Query Layer Dialog}
\end{figure}
\clearpage
\subsubsection[Details of the Query Layer]{\Large Details of the Query Layer}
\paragraph*{Enter into the tool \texorpdfstring{\\}{}}
\begin{itemize}
  \item Choose connection
  \item Name the query
  \item Enter SQl query
  \item Press Next
\end{itemize}
\begin{figure}[h!]
\centering
    \includegraphics[width=.75\textwidth]{NewQueryLayerDialogFilled.png}
\caption{Query Layer Dialog Filled}
\end{figure}
\clearpage
\subsubsection[More Details of the Query Layer]{\Large More Details of the Query Layer}
\paragraph*{Enter into the tool \texorpdfstring{\\}{}}
\begin{itemize}
  \item Select unique identifier field
  \item Click Finish
\end{itemize}
\begin{figure}[h!]
\centering
    \includegraphics[width=.95\textwidth]{SelectUniqueIdentifier.PNG}
\caption{Select Unique Identifier}
\end{figure}
\clearpage
\subsubsection[Open Results Table]{\Large Open Results Table}
\paragraph*{Verify the Query by Looking at the Table \texorpdfstring{\\}{}}
\begin{figure}[h!]
\centering
    \includegraphics[width=.95\textwidth]{QueryResultsTable.PNG}
\caption{Query Results Table}
\end{figure}
\clearpage
\end{document}
